...
// TODO createOrganizationSchema, action 'stap1' and page.svelte is nearly identical
...
export const actions: Actions = {
  stap1: async ({ params, request, cookies }) => {
    const { id } = params;
    ...
    if (!parsed.success) {
      ...
      if (formValid) {
        ...
        const message = `Lobbyorganisatie <a class='font-medium hover:underline' href='/organisaties/${organization.id}'>${organization.name}</a> is bijgewerkt`;
